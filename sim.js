...
async function showAllAPRoutes() {
    ...
    for (const ap of targets) {
        try {
            ...
            // Hasar durumuna göre renk
            const col   = ap.damage > 0.5 ? '#ef4444' : ap.damage > 0.3 ? '#f59e0b' : '#00cc66';
            const alpha = selectedAP ? 0.42 : 0.7; // AI analizi öncesi daha belirgin

            // Glow efekti
            L.polyline(latlngs, {
                renderer: cr, color: col, weight: 7, opacity: 0.12,
                dashArray: null, interactive: false,
            }).addTo(apRoutesLayer);
            // Ana çizgi
            L.polyline(latlngs, {
                renderer: cr, color: col, weight: 2.2, opacity: alpha,
                dashArray: '6 5', interactive: false,
            }).addTo(apRoutesLayer);
            // Varış noktası marker
            L.circleMarker([ap.lat, ap.lon], {
                renderer: cr, radius: 6,
